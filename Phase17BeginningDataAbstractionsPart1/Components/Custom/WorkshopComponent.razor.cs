...
public partial class WorkshopComponent(WorkshopManager manager, Inventory inventory)
{
    [Parameter]
    [EditorRequired]
    public WorkshopSummary Workshop { get; set; }
    private int _selectedIndex = 0;
    private BasicList<WorkshopRecipeSummary> _recipes = [];
    protected override void OnParametersSet()
    {
        _recipes = manager.GetRecipesForWorkshop(Workshop);
        if (_recipes.Count > 0)
        {
    ...
    private bool CanCraft => manager.CanCraft(Workshop, ChosenItem);
    private void Craft()
    {
        manager.StartCraftingJob(Workshop, ChosenItem);
    }
