namespace Phase08TestQuests.DataAccess.Workers;
internal class WorkerRecipeDatabase(PlayerState player) : ListDataAccess<WorkerRecipeDocument>
...

{
    ...
    async Task<BasicList<WorkerRecipe>> IWorkerRegistry.GetWorkersAsync()
    {
        ...
        list.ForConditionalItems(x => x.Theme == player.FarmTheme, old =>
        {
            output.Add(new()
            {
