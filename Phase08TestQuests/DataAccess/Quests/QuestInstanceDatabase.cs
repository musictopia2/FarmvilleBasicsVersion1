namespace Phase08TestQuests.DataAccess.Quests;
public class QuestInstanceDatabase(PlayerState player) : ListDataAccess<QuestDocument>
    (DatabaseName, CollectionName, mm1.DatabasePath),
    ISqlDocumentConfiguration, IQuestRecipes, IQuestPersistence

{
    public static string DatabaseName => mm1.DatabaseName;
    public static string CollectionName => "QuestInstances";
    async Task<BasicList<QuestRecipe>> IQuestRecipes.GetQuestsAsync()
    {
        var firsts = await GetDocumentsAsync();
        BasicList<QuestRecipe> output = firsts.Single(x => x.Player.Equals(player)).Quests;
        return output;
    }
    async Task IQuestPersistence.SaveQuestsAsync(BasicList<QuestRecipe> quests)
    {
        var list = await GetDocumentsAsync();
        var item = list.Single(x => x.Player.Equals(player));
        item.Quests = quests;
        await UpsertRecordsAsync(list);
    }
}