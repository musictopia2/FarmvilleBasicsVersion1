...
public class WorksiteInstance(WorksiteRecipe recipe)
{
    ...
    private static void FreeWorker(WorkerModel worker)
    {
        ...
        worker.CurrentLocation = EnumWorksiteLocation.None;
    }
    ...
    public BasicList<WorksiteRewardPreview> GetPreview(bool uiOnly = true)
    {
        ...
        HashSet<EnumItemType> candidateItems = [];
        ...
        foreach (var review in candidateItems)
        {
            ...
            if (chances == 0)
            {
                throw new CustomBasicException($"Must have at least a small chance or why bother including {review.Words}");
            }
            if (uiOnly)
            {
                chances *= 100;
    ...
    private BasicList<WorkerBenefit> GetWorkerBenefits(EnumItemType item)
    {
