...
public class AnimalManager(Inventory inventory)
{
    ...
    private IAnimalPolicy? _animalPolicy;
    ...
    {
        if (_animalPolicy is null)
        {
            return false;
        }
        var list = GetAllAnimals;
        var policy = await _animalPolicy.CanUnlockAnimalAsync(list, name);
        return policy;
    }
    public async Task UnlockAnimalAsync(string name)
    {
        var list = GetAllAnimals;
        var policy = await _animalPolicy!.UnlockAnimalAsync(list, name);
    ...
    public async Task<bool> CanLockAnimalAsync(string name)
    {
        ...
        var list = GetAllAnimals;
        var policy = await _animalPolicy.CanLockAnimalAsync(list, name);
        return policy;
    }
    public async Task LockAnimalAsync(string name)
    {
        var list = GetAllAnimals;
        var policy = await _animalPolicy!.LockAnimalAsync(list, name);
        UpdateAnimalInstance(policy);
    }
    public async Task<bool> CanIncreaseAnimalOptionsAsync(string name)
    {
        ...
        var list = GetAllAnimals;
        var policy = await _animalPolicy.CanIncreaseOptionsAsync(list, name);
        return policy;
    }
    public async Task IncreaseAnimalOptionsAsync(string name)
    {
        var list = GetAllAnimals;
        await _animalPolicy!.IncreaseOptionsAsync(list, name);
    ...
    public async Task<bool> CanDecreaseAnimalOptionsAsync(string name)
    {
        ...
        var list = GetAllAnimals;
        var policy = await _animalPolicy.CanDecreaseOptionsAsync(list, name);
        return policy;
    }
    public async Task DecreaseAnimalOptionsAsync(string name)
    {
        var list = GetAllAnimals;
        await _animalPolicy!.DecreaseOptionsAsync(list, name);
        UpdateAnimalOptions(list);
    }
    ...
    public async Task SetStyleContextAsync(AnimalServicesContext context)
    {
        _animalPolicy = context.AnimalPolicy;
        BasicList<AnimalRecipe> recipes = await context.AnimalRegistry.GetAnimalsAsync();
