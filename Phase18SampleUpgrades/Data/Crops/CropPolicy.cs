...
public class CropPolicy : ICropPolicy
{
    ...
    Task<bool> ICropPolicy.CanUnlockGrowSlotsAsync(BasicList<CropState> slots, int slotsToUnlock)
    {
        if (slots.Count == 0)
        {
    ...
    async Task ICropPolicy.UnlockGrowSlotsAsync(BasicList<CropState> slots, int slotsToUnlock)
    {
        lock (_lock)
        {
    ...
    Task<bool> ICropPolicy.CanLockGrowSlotAsync(BasicList<CropState> slots)
    {
        if (_lockedEver > 3)
        {
    ...
    async Task ICropPolicy.LockGrowSlotAsync(BasicList<CropState> slots)
    {
        lock (_lock)
        {
            var slot = slots.FirstOrDefault(x => x.State == EnumCropState.Empty && x.Unlocked);
