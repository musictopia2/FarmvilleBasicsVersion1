...
public class CropManager(Inventory inventory)
{
    ...
    private ICropPolicy? _cropPolicy;
    ...
    {
        if (_cropPolicy is null)
        {
            return false;
        }
        var policy = await _cropPolicy.CanUnlockCropAsync(_allCropDefinitions.ToBasicList(), name);
        return policy;
    }
    public Task UnlockCropAsync(string name)
    {
        return _cropPolicy!.UnlockCropAsync(_allCropDefinitions.ToBasicList(), name);
    }
    public async Task<bool> CanLockCropAsync(string name)
    {
        ...
        var policy = await _cropPolicy.CanLockCropAsync(_allCropDefinitions.ToBasicList(), name);
        return policy;
    }
    ...
    {
        return _cropPolicy!.LockCropAsync(_allCropDefinitions.ToBasicList(), name);
    }
    public async Task<bool> CanUnlockGrowSlotsAsync(int slots)
    {
        ...
        var policy = await _cropPolicy!.CanUnlockGrowSlotsAsync(list, slots);
        return policy;
    }
    public async Task UnlockGrowSlotsAsync(int slots)
    {
        var list = GetCropStates();
        await _cropPolicy!.UnlockGrowSlotsAsync(list, slots);
    ...
    public async Task<bool> CanLockGrowSlotsAsync()
    {
        ...
        var list = GetCropStates();
        var policy = await _cropPolicy!.CanLockGrowSlotAsync(list);
        return policy;
    }
    public async Task LockGrowSlotsAsync()
    {
        var list = GetCropStates();
        await _cropPolicy!.LockGrowSlotAsync(list);
        UpdateInstances(list);
    }
    ...
    {
        _cropPolicy = context.CropPolicy;
